...
| --- | --- | --- |
| `PenumbraOrganizer.Core` | Domain models, service interfaces, pure logic (no I/O) | None |
...
- **`MainViewModel`**: the orchestrator (~2,500 lines). Holds the injected Core services, the observable collections behind the grid (`ICollectionView` for filtered/selected/changed mods), and every `ICommand`. The commands map directly to UI buttons, for example `ScanCommand`, `SelectStrategyCommand`, `AssignSelectedToSelectedFolderCommand`, `CreateDryRunCommand`, `CreateBackupCommand`, `ApplyVirtualFolderChangesCommand`, `BackupAndApplyCommand`, `UndoCommand`/`RedoCommand`. If you want to understand a UI action, find its command here and follow it to the service call.
